...
var SearchableMapLib = {
  ...
  createSQL: function() {
    ...
    //-----misle search filter-----
    var misle_search = $("#search-misle").val().replace("'", "\\'");
      if (misle_search != '') {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
          return r.properties["CaseID"].toLowerCase().indexOf(misle_search.toLowerCase()) > -1;
      })}

    //-----end misle search filter-----
    // //--title search filter//
    var subtype_search = $("#search-subtype").val().replace("'", "\\'");
    if (subtype_search != '') {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
        return r.properties["Incident_Subtype"].toLowerCase().indexOf(subtype_search.toLowerCase()) > -1;
      });
    }
    //end title search filter//
    //--title search filter//
    var month_search = $("#search-month").val().replace("'", "\\'");
    if (month_search != '') {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
        return r.properties["Month"].toLowerCase().indexOf(month_search.toLowerCase()) > -1;
      });
    }

    var title_search = $("#search-title").val().replace("'", "\\'");
    if (title_search != '') {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
        return r.properties["Case_Title"].toLowerCase().indexOf(title_search.toLowerCase()) > -1;
      });
    }
  //end title search filter//
    var FY_search = $("#search-FY").val().replace("'", "\\'");
    if (FY_search != '') {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
        return r.properties["FiscalYear"].toLowerCase().indexOf(FY_search.toLowerCase()) > -1;
      });
    }

    var datetime_search = $("#search-datetime").val().replace("'", "\\'");
    if (datetime_search != '') {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
        return r.properties["1stNotificationDate/Time"].toLowerCase().indexOf(datetime_search.toLowerCase()) > -1;
      });
    }
    ...
    SearchableMapLib.currentResultsLayer = L.geoJSON(SearchableMapLib.currentResults, {
        pointToLayer: function (feature, latlng) {
          return L.marker(latlng, {icon: SearchableMapLib.getIcon(feature.properties["Incident_Subtype"])} );
        },
...
//ZOOM FUNCTION FOR WITHIN BUTTON
  setZoom: function() {
    var zoom = '';
    if (SearchableMapLib.radius >= 80470) zoom = 9; // 50 miles
    else if (SearchableMapLib.radius >= 32190) zoom = 10; // 20 miles
    else if (SearchableMapLib.radius >= 16095) zoom = 11; // 10 mile
    else if (SearchableMapLib.radius >= 8050) zoom = 12; // 5 mile
  ...
  getIcon: function(type){
    if (type == "Person in Water (PIW)") return redIcon;
    if (type == "Disabled Vessel") return blueIcon;
    if (type == "Adrift (Unmanned)") return yellowIcon;
    return greyIcon;
  },
